...
def train_each_epoch(local_class, optimizer, scheduler, model, sentence, segment, target_mask, label, batch_size, device = "cuda", common_mask = None, type_classifier = 'original'):
    model.train()
    ...
    for i in range(0, max_data_len, batch_size):  
        ...
        x = model(batch_sentence, batch_mask, batch_segment, batch_common_mask)[0]
...
def train_task(max_epoch, lr, model, data_current, batch_size, type_classifier = 'original', device = "cuda", save_name = 'news_series', save_model = False, output_dir = './model_output'): 
    ...
    while epoch <= max_epoch:
        ...
        sentence, segment, target_mask, label, common_mask = data_current['train']
        local_class = data_current['class'].to(device)
        
        acc_train = train_each_epoch(local_class, optimizer, scheduler, model, sentence, segment, target_mask, label, batch_size, device, common_mask, type_classifier)
        if epoch == max_epoch: #if epoch == True:
            sentence_val, segment_val, target_mask_val, label_val, common_mask_eval = data_current['dev']
            local_class = data_current['class'].to(device)
            eval_acc_til, eval_acc_cil, _, _, _ = evaluate_class(local_class, model, sentence_val, segment_val, target_mask_val, label_val, batch_size, device, common_mask_eval, type_classifier)
...
def train(max_epoch, lr, model, data_all, batch_size, device = "cuda", save_name = 'news_series', type_classifier = 'original', plot_rep = False):
    ...
    label = []
    for task in range(len(data_all)):
        sentence_val, segment_val, target_mask_val, label_val, common_mask_eval = data_all[task]['dev']
        local_class = data_all[task]['class'].to(device)
        save_name = data_all[task]['name']
        eval_acc_til, eval_acc_cil, rep_cur, over_smooth, sink_all = evaluate_class(local_class, model, sentence_val, segment_val, target_mask_val, label_val, batch_size, device, common_mask_eval, type_classifier)
