...
def evaluate_class(local_class, model, sentence, target_mask, label, batch_size, device = "cuda", common_mask = None, type_classifier = 'original'):
    model.eval()
    correct = 0
    correct_cil = 0
    rep = []
    ...
    for i in range(0, len(sentence), batch_size):
        ...
        with torch.no_grad():
            ...
            x, att_all, avg_cos_all, sink_all = model(batch_sentence, batch_mask, batch_common_mask, evaluate = True)
