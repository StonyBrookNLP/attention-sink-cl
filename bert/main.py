...
if __name__ == '__main__':
    ...
    for j in range(len(sequence)):
        ...
        for sub in subset:
            ...
            sent_train, seg_train, target_mask_train, spec_token, common_mask_train, common_token, vocabulary = tokenize_bert(data['train'][0], data['train'][1], encoder_type, max_len)
            label_train = data['train'][2]
                        
            sent_valid, seg_valid, target_mask_eval, _, common_mask_eval, _, _ = tokenize_bert(data['valid'][0], data['valid'][1], encoder_type, max_len)
            label_valid = data['valid'][2]
            
            sent_test, seg_test, target_mask_test, _, common_mask_test, _, _ = tokenize_bert(data['test'][0], data['test'][1], encoder_type, max_len)
            ...
            if sub is not None:
                ...
                name = dataset
            dataset_all.append({'train':(sent_train, seg_train, target_mask_train, (np.array(label_train)).tolist(), common_mask_train),
                              'dev':(sent_valid, seg_valid, target_mask_eval, (np.array(label_valid)).tolist(), common_mask_eval),
                              'test':(sent_test, seg_test, target_mask_test, (np.array(label_test)).tolist(), common_mask_test),
